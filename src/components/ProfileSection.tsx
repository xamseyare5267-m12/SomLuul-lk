...
export const ProfileSection: React.FC<ProfileSectionProps> = ({
  ...
  // Photo gallery / local mock covers and custom uploads
  ...
  // Dynamic friendship structures (Simulated inside db.json or state for sandbox)
  ...
  const fetchProfileDetails = async () => {
    setIsLoading(true);
    try {
      // 1. Fetch all profiles to list and establish relationship context
      const profRes = await axios.get('/api/profiles', {
        headers: { Authorization: `Bearer ${authToken}` },
      });
      let list: Profile[] = profRes.data;
      ...
      // If profile is not found (e.g. mock contact or handle clicked), check if we can match by handle or create a template
      if (!targetProfile) {
        // Try searching by handle
        targetProfile = list.find(p => p.email.split('@')[0] === effectiveProfileId);
      }

      if (!targetProfile) {
        // Fallback for special users or handles like 'yasmin_elmi' etc.
      ...
      // 2. Fetch all posts and filter those written by this user
      const postsRes = await axios.get('/api/posts');
      let serverPosts = postsRes.data;
      ...
      const userPosts = mergedPosts.filter((p: any) => {
        ...
        const isMyOwn = isOwnProfile && (
          ...
          p.author?.handle === user.email.split('@')[0] || 
      ...
      onShowToast('Cillad ayaa dhacday marka la soo rarayay profile-ka.', 'error');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleFriendshipAction = async () => {
    ...
    try {
      // Simulate state update on server or local DB
      // We send request, accept, or remove
      let nextState: 'none' | 'sent' | 'received' | 'friends' = 'none';
      let msg = '';

      if (friendshipState === 'none') {
        nextState = 'sent';
        msg = 'Codsiga saaxiibtinimo waa la diray!';
      } else if (friendshipState === 'sent') {
        nextState = 'none';
        msg = 'Codsiga saaxiibtinimo waa laga laabtay.';
      } else if (friendshipState === 'received') {
        nextState = 'friends';
        msg = 'Hadda waxaad tihiin saaxiibo!';
      } else if (friendshipState === 'friends') {
        nextState = 'none';
        msg = 'Waa laga saaray saaxiibada.';
      }

      setFriendshipState(nextState);
      onShowToast(msg, 'success');

      // Update custom fields locally in DB via silent update
      const updatedUserFriends = [...(user.friends || [])];
      const updatedUserRequests = [...(user.friendRequests || [])];

      if (nextState === 'friends') {
        if (!updatedUserFriends.includes(profile.id)) updatedUserFriends.push(profile.id);
        const requestIdx = updatedUserRequests.indexOf(profile.id);
        if (requestIdx > -1) updatedUserRequests.splice(requestIdx, 1);
      } else if (nextState === 'sent') {
        if (!updatedUserRequests.includes(profile.id)) updatedUserRequests.push(profile.id);
      } else if (nextState === 'none') {
        const friendIdx = updatedUserFriends.indexOf(profile.id);
        if (friendIdx > -1) updatedUserFriends.splice(friendIdx, 1);
        const reqIdx = updatedUserRequests.indexOf(profile.id);
        if (reqIdx > -1) updatedUserRequests.splice(reqIdx, 1);
      }

      // Sync with user layout profile
      const finalUserUpdate = {
        ...user,
        friends: updatedUserFriends,
        friendRequests: updatedUserRequests,
      };
      onProfileUpdate(finalUserUpdate);
      ...
      console.warn('Friendship action server syncing failed', err);
    }
  };
