...
export const UserProfileSidebar: React.FC<UserProfileSidebarProps> = ({
  ...
  const targetProfileId = matchingRealProfile?.id || room?.id || 'r_maryan';

  // Derive meta profiles values
  const currentBio = matchingRealProfile?.bio || room?.bio || 'Halkan kuma qorna wax Taariikh Nololeed ah.';
  const currentPhone = matchingRealProfile?.phone || room?.phone || '+252 61 7770000';
  ...
  // Custom mock values to fill stats perfectly with zero placeholders
  ...
  const sharedGroupsCount = room.isGroup ? 1 : Math.floor(Math.random() * 3) + 1;
  const sharedMediaCount = Math.floor(Math.random() * 18) + 4;
  const mutualContactsCount = Math.floor(Math.random() * 5) + 1;
