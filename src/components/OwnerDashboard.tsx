...
export const OwnerDashboard: React.FC<OwnerDashboardProps> = ({
  ...
  // Web Landing customization states
  ...
  useEffect(() => {
    ...
    // Auto refresh status metrics and printer state
    const interval = setInterval(() => {
      setSystemStats(prev => ({
        ...prev,
        serverCPU: Math.max(8, Math.min(95, prev.serverCPU + Math.floor(Math.random() * 7) - 3)),
        serverRAM: Math.max(38, Math.min(85, prev.serverRAM + Math.floor(Math.random() * 3) - 1)),
        activeChats: Math.max(10, prev.activeChats + Math.floor(Math.random() * 5) - 2),
      }));
          ...
          {activeSubTab === 'analytics' && (
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {/* Traffic Activity Simulation Chart */}
                <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-sm font-bold text-white">Live Operations Traffic</h4>
                      <p className="text-xs text-slate-400">Simulating message events per minute</p>
                    </div>
                    <span className="text-[10px] font-mono bg-blue-500/15 text-blue-400 border border-blue-500/20 px-2 py-0.5 rounded-full font-bold">REALTIME</span>
                  </div>
                  <div className="h-40 flex items-end gap-1.5 pt-4">
                    {[32, 45, 21, 64, 87, 54, 76, 43, 65, 87, 98, 76, 54, 87, 92, 43, 67, 89, 99, 120].map((val, idx) => (
                      <div key={idx} className="flex-1 flex flex-col justify-end items-center h-full">
                        <div 
                          className="w-full rounded-t bg-rose-500/80 hover:bg-rose-400 transition-all duration-300 relative group cursor-default"
                          style={{ height: `${(val / 120) * 100}%` }}
                        >
                          <span className="absolute -top-6 left-1/2 -translate-x-1/2 scale-0 group-hover:scale-100 transition-all bg-slate-900 border border-slate-700 px-1.5 py-0.5 rounded text-[8px] font-mono text-white">
                            {val}
                          </span>
                        </div>
                        <span className="text-[8px] text-slate-600 font-mono mt-2">{idx * 2}m</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* System Status Table */}
                <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-4">
                  <h4 className="text-sm font-bold text-white">Interactive Operational Health</h4>
                  <div className="space-y-3 font-mono text-xs">
                    ...
                    <div className="flex justify-between items-center p-2.5 bg-slate-900 rounded-xl border border-slate-800">
                      <span className="text-slate-400">Redis Cache Clusters</span>
                      <span className="text-emerald-400 font-bold flex items-center gap-1.5">
                        <span className="w-2 h-2 rounded-full bg-emerald-500"></span> 3 Node Online
                      </span>
                    </div>
                    <div className="flex justify-between items-center p-2.5 bg-slate-900 rounded-xl border border-slate-800">
                      <span className="text-slate-400">Object Storage (MinIO / Cloud Storage)</span>
                      <span className="text-emerald-400 font-bold flex items-center gap-1.5">
                        <span className="w-2 h-2 rounded-full bg-emerald-500"></span> 2.4 GB Used
                      </span>
                    </div>
                    <div className="flex justify-between items-center p-2.5 bg-slate-900 rounded-xl border border-slate-800">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-4">
                  ...
                  <div className="space-y-4 text-xs">
                    ...
                    {/* Preset Logos */}
                    <div>
                      <label className="block text-[10px] font-bold text-slate-500 uppercase font-mono mb-1.5">Presets diyaar ah (Sariiraha Logada)</label>
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => setRemoteConfig({ ...remoteConfig, appLogo: 'https://images.unsplash.com/photo-1614064641938-3bbee52942c7?w=120' })}
                          className={`px-2.5 py-1.5 rounded-lg border text-[10px] font-bold transition-all ${
                            remoteConfig.appLogo?.includes('photo-1614064641938-3bbee52942c7')
                              ? 'bg-amber-500/10 text-amber-400 border-amber-500/40'
                              : 'bg-slate-900 text-slate-400 border-slate-800 hover:text-white'
                          }`}
                        >
                        ...
                        <button
                          type="button"
                          onClick={() => setRemoteConfig({ ...remoteConfig, appLogo: 'https://images.unsplash.com/photo-1541701494587-cb58502866ab?w=120' })}
                          className={`px-2.5 py-1.5 rounded-lg border text-[10px] font-bold transition-all ${
                            remoteConfig.appLogo?.includes('photo-1541701494587-cb58502866ab')
                              ? 'bg-amber-500/10 text-amber-400 border-amber-500/40'
                              : 'bg-slate-900 text-slate-400 border-slate-800 hover:text-white'
                          }`}
                        >
                          Abstract Aura Purple
                        </button>
                      </div>
                    </div>
                  </div>
                ...
                <div className="lg:col-span-7 space-y-6">
                  ...
                  <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <button
                        onClick={async () => {
                          ...
                          try {
                            ...
                            onShowToast(e.response?.data?.error || 'Simulated print failed.', 'error');
                          } finally {
                            setIsPrinting(false);
                          }
                        }}
                      ...
                      >
                        ...
                        Tijaabi Offline Retry (Simulation)
                      </button>
                    ...
                    <div className="p-3 bg-rose-500/5 border border-rose-500/10 rounded-xl space-y-1 text-[11px] text-slate-400 leading-relaxed font-sans">
                      ...
                      Ku dhufo badhanka <strong className="text-rose-300">"Tijaabi Offline Retry (Simulation)"</strong>.
                  ...
                  <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-4">
                    <h3 className="font-bold text-white text-sm uppercase tracking-wider font-mono flex items-center gap-2 border-b border-slate-800 pb-3">
                      ...
                      Simulate Social Webhook Cloud Print
                    </h3>
                    <p className="text-xs text-slate-400">
